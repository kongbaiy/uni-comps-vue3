<template>
  <view v-if="show" :style="{
    ...
    <custom-icon v-if="showCloseIcon && !$slots.icon" type="close" :size="42" class="popup-close" @tap="handleClose" />
  ...
  <view v-if="showMask && show" :class="[
    'popup-mask'!,
    active ? 'popup-mask__active' : '',
  ]" catchtouchmove="emptyFunction" @click="maskClose && handleClose()" />
</template>
